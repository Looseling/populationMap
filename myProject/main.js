...
(async function () {
  popData = await getPopulationData();
  ...
  controls.enablePan = false; // This line prevents panning
  ...
  //test
  //rain
  const rainDropVelocity = new THREE.Vector3(0, -3, 0);
  ...
  function animateRaindrops(currentIndex) {
    ...
    for (let i = 0; i < country.raindropStates.length; i++) {
      ...
      if (!state.isFalling && Math.random() > 0.999) state.isFalling = true;
      if (state.isFalling) {
        state.position.add(state.velocity);
        const highest =
          country.highestVoxels.get(
            `${state.position.x},${state.position.z}`
          ) || 0;
        if (state.position.y <= highest + 3 || state.position.y == 0) {
          state.position.y = highest + 0.5;
  ...
  // Check if screen size is less than or equal to 600px
  if (window.matchMedia("(max-width: 600px)").matches) {
    // The viewport is less than or equal to 600px
    // You can disable the chart here
    const chartElement = document.getElementById("acquisitions");
    chartElement.style.display = "none";
  } else {
    // The viewport is greater than 600px
    // You can enable the chart here
    const chartElement = document.getElementById("acquisitions");
  ...
  async function getPopulationData() {
    try {
      const response = await fetch("./PopulationData.txt");
  ...
  async function DataColumn(country) {
    ...
    if (chartInstance) {
      ...
      chartInstance = new Chart(document.getElementById("acquisitions"), {
        ...
        options: {
          responsive: true, // Ensures that your chart will resize when the parent container changes size.
